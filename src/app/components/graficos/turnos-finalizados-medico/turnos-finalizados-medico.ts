...
@Component({
  ...
  templateUrl: './turnos-finalizados-medico.html',
  styleUrl: './turnos-finalizados-medico.css'
})
export class TurnosFinalizadosMedico {
chartData: ChartConfiguration<'pie'> | null = null;
  ...
  ngOnInit() {
    this.us.traerTurnos().then(({ data, error }) => {
      if (data) {
        const turnosFiltrados = data.filter(t => t.estado === EstadoTurno.Finalizado);
        this.turnosTotales = turnosFiltrados;
  ...
  descargarInformePdf() {
    ...
    doc.setFontSize(20);
    doc.text(`Informe de turnos (FINALIZADOS) por Medico (${this.startDate} al ${this.endDate}):`, 105, 50, { align: "center" });
